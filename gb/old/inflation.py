...
def M_P_small(P, rho):
  return (P * (rho * 4 / 3 * Pi())**(2 / 3) / (2 / 3 * Gval() * rho**2 * Pi()))**(3 / 2)

# '----based on second revision of mathematics ---
# ' Functions based on (M,R,t) tripple
def M_Rt(R, T, rho):
  return (4. / 3.) * Pi() * rho * ((R + T)**3 - R**3)

def R_Mt(M, T, rho):
  return (-T + (T**2 - 4 * ((T**2) / 3 - M / (4 * Pi() * rho * T)))**(1 / 2)) / (2)
...
# ' Composite function
def dPdR_RM(R, M, rho):
  return dPdR_Rt(R, t_RM(R, M, rho), rho) + dPdt_Rt(R, t_RM(R, M, rho), rho) * dtdR_RM(R, M, rho)

# ' Functions based on (P,R,t) tripple
def P_Rt(R, T, rho):
  return (2. / 3) * Pi() * Gval() * (T * rho)**2 * (3 * R + T) / (R + T)

def dPdt_Rt(R, T, rho):
  return (4. / 3) * rho**2 * Pi() * Gval() * T * (3 * R**2 + 3 * R * T + T**2) / (R + T)**2

def dPdR_Rt(R, T, rho):
  return 4 * T**3 * Gval() * Pi() * rho**2 / (3 * (R + T)**2)

def R_Pt(P, T, rho):
  return (1. / 3) * T * (3 * P - 2 * rho**2 * Pi() * Gval() * T**2) / (-P + 2 * rho**2 * Pi() * Gval() * T**2)
...
def R_MP(M, P, rho):
  ...
  while True:
    ...
    R_MP = R_MP + delta
    n = n + 1
    if (abs(delta * rho / (P / (2 * Gval() * Pi())))**(1 / 2) < 1e-06):  # Then    ' divide by est. value of t
      break
    else: (n > 20):
      R_MP = -2
      break
...
def P_RS(R, S, rho):
  return (2. / 3) * Pi() * Gval() * rho**2 * (S - R)**2 * (S + 2 * R) / S
